...
def check_style(config):
    ...
    if child.returncode != 0 or len(extract_stdout(child)) != 0:
        ...
        coding_style_grade = min(config.grade/config.tests_points * 10, 10)
    ...
    if not is_online_judge() and should_play_sound:
...
def run_task(config, task):
    ...
    for test in task.tests:
        ...
        log('----- running test {} - grade {:g}/{:g}'.format(test.id,
...
def print_grade_vmr(config):
    pass # left over from old vmchecker - TODO: remove me and all logic that inserts into config.grade_vmr
...
def checker_run():
    ...
    if not is_online_judge() and config.grade >= 100:
