...
check_coding_style()
{
    ...
    declare -a IGNORED_FLAGS=(
        ...
        BRACES                      # kernel specific convention
        EMBEDDED_FUNCTION_NAME      # DN -> not required for PCLP: Prefer using '"%s...", __func__' to using 'main', this function's name, in a string
        MALFORMED_INCLUDE           # DN -> not required for PCLP: ERROR:MALFORMED_INCLUDE: malformed #include filename
        CONSTANT_COMPARISON         # DN -> not required for PCLP: WARNING:CONSTANT_COMPARISON: Comparisons should place the constant on the right side of the test
        TYPO_SPELLING               # DN -> not required for PCLP: CHECK:TYPO_SPELLING: 'alocate' may be misspelled - perhaps 'allocate'?
    )
...
if [ $# -eq 0 ] || [ "$1" = "-h" ]; then
    ...
    find "${TARGET_DIR}" -type f \( -iname '*.c' -o -iname '*.h' \)  -exec bash -c 'check_coding_style "{}"' \;
fi
